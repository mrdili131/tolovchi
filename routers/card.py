...
@router.get('/transactions',response_model=list[TransactionResponse],status_code=200,summary="Gets transactions based on role")
async def get_transactions(db: Session, user: user_dependency):
    transactions = await db.scalars(select(Transaction).filter(
        Transaction.sender_id==user.get("id") or Transaction.receiver_id==user.get("id"))
        .options(
